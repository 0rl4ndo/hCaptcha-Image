import httpx,json,datetime,urllib,threading,base64,hashlib,math,datetime,requests,os,binascii,time,itertools,fade,string,random


__proxies__,__config__ = itertools.cycle(list(set(open('proxies.txt', 'r+').read().splitlines()))), json.load(open('config.json'))
...
class hCaptcha(threading.Thread):
    def __init__(self, proxy: str = None if __config__['proxyless'] else f'http://{next(__proxies__)}', site_key: str = '4c672d35-0701-42b2-88c3-78380b0db560', website_url: str = 'discord.com') -> None:
        self._client = httpx.Client(proxies=proxy, timeout=3000)
    ...
    def _Get_Task(self):
        ...
        response = self._client.post(f'https://hcaptcha.com/checksiteconfig?v=1f7dc62&host={self.website}m&sitekey={self.site_key}&sc=1&swa=1')
        try:self._client.headers.pop('Content-Length') 
        except:pass

        return response.json()

    def _Get_Captcha(self, C, N):
        p = {
            'v': '1f7dc62',
    ...
    def Label_Cleaner(self, Label):
        Bad_Code = {"а": "a","е": "e","e": "e","i": "i","і": "i","ο": "o","с": "c","ԁ": "d","ѕ": "s","һ": "h","у": "y","р": "p"}
        Label = Label.replace(".", "").lower()
        clean_label = Label
        for c in Bad_Code:clean_label = clean_label.replace(c, Bad_Code[c])
        return clean_label

    def _Download_URL(self, URL, Label):
        global Downloaded,Duplicates
        try:
            data = requests.get(URL,stream=True).content
            if data in No_Duplicate:print(f'\x1b[38;5;63m[\x1b[0mDEBUG\x1b[38;5;63m]\x1b[0m Duplicate Founded.    --> {Label}');Duplicates += 1;return 

            folder = os.path.join(f'images/{Label}')
            if not os.path.isdir(folder):os.mkdir(folder)
            with open(f"images/{Label}/{binascii.b2a_hex(os.urandom(15)).decode('utf-8')}_{''.join(random.sample(string.ascii_letters+string.ascii_lowercase+string.ascii_uppercase, 8))}_{base64.b64encode(str(datetime.datetime.now()).encode()).decode()}.jpg","wb") as f:f.write(data)
            No_Duplicate.append(data);Downloaded += 1;print(f'\x1b[38;5;63m[\x1b[0mDEBUG\x1b[38;5;63m]\x1b[0m Downloaded New Image. --> {Label}')

        except Exception as e:print(e)

    def run(self):
            try:#print(Task)
                Task = self._Get_Task();Task['c']['type'] = 'hsl';n = self._HSL(Task['c']['req']); Cap = self._Get_Captcha(C=Task['c'],N=n)
                if not 'containing a ' in Cap['requester_question']['en']: obj = Cap['requester_question']['en'].split('Please click each image containing an ')[1].strip()
                else: obj = Cap['requester_question']['en'].split('Please click each image containing a ')[1].strip()
                # lol 
                obj = str(self.Label_Cleaner(str(obj)))
                for i in Cap['requester_question_example']:
                    self._Download_URL(i, obj.replace(" ", "_"))
            except Exception as e: print(e)


if __name__ == '__main__':
    print(fade.purplepink("""\n    ╦ ╦╦ ╦╦ ╦\n    ║ ║║║║║ ║\n    ╚═╝╚╩╝╚═╝\n    """))
    No_Duplicate = [];Downloaded = 0;Duplicates = 0
    def title():
        while True: os.system(f'title hCaptcha Image Downloader ^| Downloaded: {Downloaded} ~ Duplicates: {Duplicates}') if os.name == 'nt' else None
    for i in os.listdir('images'): 
        for f in os.listdir(f'images\{i}'):
            with open(f'images\{i}\{f}', "rb") as file:data = file.read()
            No_Duplicate.append(data) 

    threading.Thread(target=title).start()        
    
    while True:
        try: hCap = hCaptcha();hCap.start();time.sleep(0.001)
        except Exception as e: print(e)
